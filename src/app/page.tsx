            ...
            <SwiperSlide key={index} className="h-full">
              ...
              >
                ...
                <div className="h-full w-full flex flex-col justify-center items-center md:text-center px-5 space-y-2 md:space-y-5">
                  <h1 className="md:text-6xl text-2xl text-[29px] font-bold text-white z-10">
                    {slide.title}
                  </h1>
                  <p className="md:text-2xl text-md w-full text-white z-10">
                    {slide.subtitle}
                  </p>
                </div>
              </div>
      ...
      <section id="about">
        ...
        <div className="flex flex-col md:flex-row px-5 md:px-10 gap-8 ">
          ...
          <div className="flex flex-col justify-center">
            <p className="text-lg">
              Welcome to Mod Kitchen Equipment Pvt Ltd! We are your trusted
              partner in creating exceptional culinary spaces. Whether you run a
              bustling hotel, a cozy restaurant, or a large-scale industrial
              kitchen, we&apos;ve got you covered. <br /> <br />
              At Mod Kitchen, we&apos;re passionate about elevating your kitchen
              experience. We understand that the heart of any food establishment
              lies in its kitchen. That&apos;s why we offer top-of-the-line
              commercial kitchen equipment that combines functionality,
              aesthetics, and durability.
            </p>
            <div className="flex md:gap-7 gap-3 my-5 text-center flex-col md:flex-row text-secondary text-lg ">
              <Link
                href={"/e-catalog.pdf"}
                className="bg-foreground px-5 py-2 rounded-md md:w-2/5"
              >
                Read More
              </Link>
              <Link
                href={"/contact-us"}
                className="bg-primary px-5 py-2 rounded-md md:w-2/5"
              >
                Contact Us
              </Link>
            </div>
        ...
        <div className="md:flex grid grid-cols-2 md:flex-row md:flex-wrap md:gap-16 gap-5 px-5 justify-center ">
          {whyus.map((item, index) => (
            <div key={index} className="flex flex-col items-center gap-3">
              ...
              <h5 className="text-base md:text-2xl font-medium text-center">
                {item.title}
              </h5>
            </div>
          ))}
      ...
      <section className="md:mt-28 md:mb-20 my-20 max-w-screen-xl mx-auto">
        ...
        <div className="md:columns-3 space-y-6">
          {testimonials.map((t, index) => (
            <div
              key={index}
              className="  p-6 bg-accent border-2 rounded-lg shadow-md"
            >
